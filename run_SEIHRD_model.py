...
def plot_cases(projection, dataset, state, initial_date, ls='-', label_proj='projection', label_data=''):
    ...
    plt.plot(data.values, lw=3, label=label_data, c='red')
...
def sol_to_csv(filename, sol, initial_date):
...
def SEIHRD_markov_step(x, t, S_tc, CH_tc, *params):
    '''
...
def ICUcases(sol): return Hospitalized(sol) + Recovered(sol) + Deceased(sol)
## Aggregation
...
if run:
    if __name__ == "__main__":
        ...
        ## Preallocation ##
        # Vector of initial conditions. One set of initial conditions for each state
        x0_vec = []
        ...
        for (i,state) in enumerate(states_mex):
            ...
            r = 0.7 # r denotes the fraction of latent cases with respect to the confirmed cases. This is, E0 + A0 = r * I0
            ...
            E0 = 1*r/2 * I0                 # fraction of exposed non-infectious cases. Latent variable
            A0 = 1*r/2 * I0                 # fraction of asymptomatic but infectious cases. Latent variable
            ...
            # append state_i to the vector of initial conditions
            x0_vec.append(x0)
            ...
            print('Projecting for {}'.format( state ))
            projection_state_i_scenario_1 = solve(SEIHRD_markov_step, x0_vec[i], 0.0, n_days, *params_1) * N
            projection_state_i_scenario_2 = solve(SEIHRD_markov_step, x0_vec[i], 0.0, n_days, *params_2) * N
            projection_state_i_scenario_3 = solve(SEIHRD_markov_step, x0_vec[i], 0.0, n_days, *params_3) * N
        ...
        ### SAVING RESULTS ###
        PLOT_PATH = './media/covid19_mex_proyecciones.png'
        CSV_PATH  = './results/'
        ...
        sol_to_csv(CSV_PATH+'covid19_mex_proyeccion_susana_00.csv', projections_susana1, initial_date)
        sol_to_csv(CSV_PATH+'covid19_mex_proyeccion_susana_20.csv', projections_susana2, initial_date)
        sol_to_csv(CSV_PATH+'covid19_mex_proyeccion_susana_50.csv', projections_susana3, initial_date)
